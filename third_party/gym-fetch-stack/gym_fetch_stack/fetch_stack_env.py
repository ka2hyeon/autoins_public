...
class FetchStackEnv(robot_env.RobotEnv):
    ...
    def __init__(
        ...
        distance_threshold, initial_qpos, reward_type, goals_on_stack_probability=1.0, allow_blocks_on_stack=True,
            all_goals_always_on_stack=False, pretraining = False
    ):
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        ag1 = achieved_goal[0:3]
        ag2 = achieved_goal[3:6]
        
        is_xy_aligend = np.linalg.norm(ag1[0:2]-ag2[0:2]) < self.distance_threshold
        is_z_aligned = (np.linalg.norm(ag2[2]-ag1[2]) < 2*self.distance_threshold) and (ag2[2]>ag1[2])
        is_ag1_achieved = distances[0] < self.distance_threshold
        is_ag2_achieved = distances[1] < self.distance_threshold

        is_ag2_is_on_ag1 = is_xy_aligend and is_z_aligned
        #far_from_gaols = self.gripper_pos_far_from_goals(achieved_goal, goal)
       
        if self.reward_type == 'pretraining': 
            ...
            # exp3 : reward = -np.min(distances)
            # exp4 : reward = reward
            #reward = np.mean([-(d > self.distance_threshold).astype(np.float32) for d in distances], axis=0)
            ...
            if is_ag2_achieved:
                reward +=1
            if (is_ag1_achieved and is_ag2_achieved):
                reward +=1
            #reward = -np.sum(distances)
            reward = np.asarray(reward)
            return reward
            ...
            #    import IPython
            #    IPython.embed()
            #np.putmask(reward, reward == 0, far_from_gaols)
            reward = np.asarray(reward) 
            return reward
    ...
    def _sample_goal(self, return_extra_info=False):
        ...
        ## ka2hyeon added
        if self.reward_type == 'pretraining':
            number_of_goals_along_stack = 0
            #goal0[2] = np.max([goal0[2], self.height_offset])
            goal0[2] = self.height_offset
        else:
            goal0 = np.array([1.28941253, 0.64580896, self.height_offset])
            number_of_goals_along_stack = 2
        ...
        for i in range(self.num_blocks - 1):
            if i < number_of_goals_along_stack - 1:
                ...
                if self.reward_type == 'pretraining': 
                    ...
                    #goal_i[2] = np.max([goal_i[2], self.height_offset])
                    goal_i[2] = self.height_offset
                else:
                    goal_i += self.target_offset
                    goal_i[2] = self.height_offset

                # TODO was 0.2 ????? 2021/09/06 commented
        ...
        #################################
        #goals = []
        #goals.append(np.array([1.28941253, 0.64580896, self.height_offset]))
        #goals.append(np.array([1.28941253, 0.8080896, self.height_offset]))
        #goals.append([0.0, 0.0, 0.0])
